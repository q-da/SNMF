...
function [y, A, evs] = CAN1(X, c, k, r, islocal)
...
distX= NormalizeFea(distX);
distX =1./(-log(distX+eps));
[distX1, idx] = sort(distX,2);%每行元素从小到大排序
A = zeros(num);
rr = zeros(num,1);
for i = 1:num
    di = distX1(i,2:k+2);
...
[clusternum, y]=conncomp(graph(sparse(A))); y = y';
if clusternum ~= c
